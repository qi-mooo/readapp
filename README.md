...
> ℹ️ **后端版本兼容性**：当前客户端使用 `/api/5` 路径与轻阅读后端通讯，具体见 `APIService.apiVersion`。如果后端升级了 API 版本（例如从 v5 升到 v6），只需在 `ReadApp/Services/APIService.swift` 中更新该常量，客户端就会自动适配新的接口路径。
